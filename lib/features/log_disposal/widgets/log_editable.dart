...
class _LogEditableFieldsState extends State<LogEditableFields> {
  ...
  void initState() {
    ...
    productInfoController = TextEditingController(
      text: widget.logEntry.productInfo,
    );
    notesController = TextEditingController(text: widget.logEntry.notes ?? '');
    quantityController = TextEditingController(
      text: widget.logEntry.quantity ?? '',
    );
    proTipController = TextEditingController(
      text: widget.logEntry.disposalGuideProTip,
    );
    toDoController = TextEditingController(
      text: widget.logEntry.disposalGuideToDo.join('\n'),
    );
    notToDoController = TextEditingController(
      text: widget.logEntry.disposalGuideNotToDo.join('\n'),
    );
  ...
  void _notifyUpdate() {
    final updatedEntry = widget.logEntry.copyWith(
      ...
      disposalGuideToDo:
          toDoController.text.split('\n').map((e) => e.trim()).toList(),
      disposalGuideNotToDo:
          notToDoController.text.split('\n').map((e) => e.trim()).toList(),
        ...
        items: [
          ...
          DropdownMenuItem(
            ...
            child: Row(
              children: [
                Image.asset(
                  'assets/images/icons/battery-blue.png',
                  width: 24,
                  height: 24,
                ),
                const SizedBox(width: 8),
          ...
          DropdownMenuItem(
            ...
            child: Row(
              children: [
                Image.asset(
                  'assets/images/icons/others.png',
                  width: 24,
                  height: 24,
                ),
                const SizedBox(width: 8),
